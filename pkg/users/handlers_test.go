...
func TestCreate(t *testing.T) {
	...
	_, err = users.Post(url, data{})
	if err != nil && err.Error() != "invalid user" {
		t.Errorf("Create: user.Create() error=%s", err.Error())
	}
	if err == nil {
		t.Errorf("Create: user.Create() failed")
	}
	...
	if err != nil && err.Error() != "invalid email, name or password" {
		t.Errorf("Create: user.Create() error=%s", err.Error())
	}
	if err == nil {
		t.Errorf("Create: user.Create() failed")
	...
	// Create new user
	newUser, err := users.Post(url, data{
		User: user,
	})
	if err != nil {
...
func TestGet(t *testing.T) {
	...
	createUserURL := fmt.Sprintf("%s/api/v1/users", baseURL)

	// Create new user
	newUser, err := users.Post(createUserURL, data{
		User: user,
	})
	if err != nil {
		t.Errorf("Create: client.Create(user) failed=%s", err.Error())
	...
	_, err = users.Get(fmt.Sprintf("%s/api/v1/users/%s", baseURL, "invalid_id"))
	if err == nil {
		t.Errorf("TestGet: client.Get(id) failed")
	}

	getUserURL := fmt.Sprintf("%s/api/v1/users/%s", baseURL, newUser.ID)

	// Test Get new user
	getUser, err := users.Get(getUserURL)
	if err != nil {
